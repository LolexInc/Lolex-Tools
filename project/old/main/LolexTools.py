...
try:
    ...
    LolexToolsMethods.bak("exitsettings", "./Lolex-Tools/project/old/", 0, 0, 1)
    restart = True
if restart == True:
...
#except(ImportError, SyntaxError, TabError):
    ...
    #update = False
        ...
        #while arraypos != len(requiredpatches.patches):
            #print(arraypos)
            #if os.system(LolexToolsMethods.pyo + " ./Lolex-Tools/project/old/update" + requiredpatches.patches[arraypos]  + ".py") != 0:
                #print("Couldn't update to " + requiredpatches.patches[arraypos] +": Failed to run update script.")
                #time.sleep(5)
                #os._exit(0)
            #arraypos = arraypos + 1
        #try:
            #os.remove("./Lolex-Tools/project/old/patches.py")
        #except(IOError, OSError):
            #pass
        #with open("./Lolex-Tools/project/old/patches.py", "a") as outf: 
            #outf.write('applied = ')
...
try:
    ...
    if LolexToolsOptions.username1 == usernameenter:
        ...
        if LolexToolsOptions.onepintotal != 0:
            with open ("./Lolex-Tools/project/old/onepinner.py", "w+") as outf:
                outf.truncate()
        ...
        if verifonboot.oneswapwords == True:
            ...
            if LolexToolsOptions.onewordtotal != 0:
                with open("./Lolex-Tools/project/old/oneworder.py", "w+") as outf:
                    outf.truncate()
        ...
        if LolexToolsOptions.twopintotal != 0:
            with open ("./Lolex-Tools/project/old/twopinner.py", "w+") as outf:
                outf.truncate()
        ...
        if verifonboot.twoswapwords == True:
            ...
            if LolexToolsOptions.twowordtotal != 0:
                with open ("./Lolex-Tools/project/old/twoworder.py", "w+") as outf:
                    outf.truncate()
    ...
    if (verifonboot.runtimeone != runtimeone) or (verifonboot.runtimetwo != runtimetwo) or (verifonboot.oneswappins != oneswappins) or (verifonboot.twoswappins != twoswappins) or (verifonboot.wordtimeone != wordtimeone) or (wordtimetwo != verifonboot.wordtimetwo) or (oneswapwords != verifonboot.oneswapwords) or (twoswapwords != verifonboot.twoswapwords):
        with open ("./Lolex-Tools/project/old/verifonboot.py", "w+") as outf:
            outf.truncate()
    ...
    while True:
        ...
        if modewanted == 1:
            ...
            if setting == "1":
                ...
                with open ("./Lolex-Tools/project/old/menusettings.py", "w+") as outf:
                    outf.truncate()
                    outf.write("layout = ")
            ...
            startplugins.uptime()
                ...
                #print("Upon prompt for saving the file, please save as Lolex-Tools-master.zip in your Lolex-Tools folder.")
                        ...
                        #continueto = int(input("Git was not found. Please press 1 to initiate webbrowser method or 0 to cancel."))
                                ...
                                #try:
                                        #os.remove("./Lolex-Tools/project/old/newversion")
                                #except(IOError, OSError):
                                        #pass
                                #os.mkdirs("./Lolex-Tools/project/old/newversion")
                                #newver = os.listdirs("./Lolex-Tools/project/old/newversion")
        # search for zips instead :P
                                ##zip_ref = zipfile.ZipFile("./Lolex-Tools/project/old/newversion"+newver[0], "r")
                                #print("Extracting...")
